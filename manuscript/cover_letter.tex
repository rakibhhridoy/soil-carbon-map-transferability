...
\begin{itemize}
  ...
  better between continents under the identical protocol. Constructively, on the order of
  ten to twenty-five local cores recover most of the lost skill, and this recovery comes
  overwhelmingly from the local data rather than from transfer, so the binding constraint
  for an unsampled delta is local sampling rather than a better global map.
